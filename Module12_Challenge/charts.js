...
function buildCharts(sample) {
  ...
  // 2.1 Use d3.json to load and retrieve the samples.json file 
  d3.json("samples.json").then((data) => {

    // 3.1 Create a variable that holds the samples array. 
    var samplesData = data.samples;
    console.log(samplesData); 

    // 4.1 Create a variable that filters the samples for the object with the desired sample number.
    ...
    //  5.1 Create a variable that holds the first sample in the array.
    var result = resultArray[0];
    console.log(result);
    ...
    // 6.1 Create variables that hold the otu_ids, otu_labels, and sample_values.
    ...
    // 7.1 Create the yticks for the bar chart.
    // Hint: Get the the top 10 otu_ids and map them in descending order  
    //  so the otu_ids with the most bacteria are last. 

    var yticks = otu_ids.slice(0,10).map(function(id){return `OTU ${id}`}).reverse();
    console.log(yticks); 
    
    // 8.1 Create the trace for the bar chart. 
    var barData = [{
    ...
    // 9.1 Create the layout for the bar chart. 
    var barLayout = {
     title: "Top 10 Bacteria Cultures Found",
     xaxis: {title: "Sample Values"},
     yaxis: {title: "Bacterial Species ID"}
    };
    // 10.1 Use Plotly to plot the data with the layout. 
